...
- exact: strong support for the most specific requested population/intervention; membership of the broad primary segment alone is not an exact query match;
...
- Plan each analysis from the broad first group into its more selective subgroups. Start with a useful umbrella-group result, then examine the requested aspects below it; use stratification when phase, design, disease or endpoint differences make a pooled estimate misleading.
...
Return only the structured SAP and variable plan."""
    ...
    async def analyze_objective(
        ...
        *,
        ...
        from intel_mcp.max_group_analysis import examined_groups, check_group_assessments, publication_dispositions, order_group_findings
...
- Lead every analysis with the first planned group's supported broad result, then place more selective group results directly below it in planned order. Within each metric, show the broad context before subgroup detail, including detailed useful subgroup findings. A broad result can be a stratified overview instead of a misleading pooled average. Label each group explicitly and preserve actual analytical denominators; never inflate N to the report-wide total or add unrelated trials to reach the cohort cap. If no meaningful broad result exists for this objective, retain the useful subgroup result without filler. Keep chart wording independent of display position (avoid "the first bar" or similar references).
...
Return only structured report content."""
        ...
        result = order_group_findings(result, segment_metadata)
