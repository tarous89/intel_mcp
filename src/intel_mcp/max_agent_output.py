...
def validate_facts(work, profiles, documents):
    valid, errors = {}, []
    entries = work.get("facts", [])
    if not isinstance(entries, list):
        return {}, [{"code": "unsupported_fact", "id": ""}]
    for fact in entries:
...
def review(work, profiles, documents, plan):
    if not isinstance(work, dict):
        return {}, [{"code": "invalid_work"}]
    facts, issues = validate_facts(work, profiles, documents)
    # Validation must not rewrite saved input or invalidate unaffected sections.
    members = copy.deepcopy(work.get("membership", {}))
    if not isinstance(members, dict):
        members = {}
        issues.append({"code": "invalid_membership"})
    ...
    for group, assignment in list(members.items()):
        if group not in group_ids or not isinstance(assignment, dict):
            members.pop(group)
            issues.append({"code": "invalid_membership", "groupId": group})
            continue
        for tid, state in list(assignment.items()):
            if tid not in profiles or state is not None and type(state) is not bool:
                assignment.pop(tid)
            ...
            try:
                ...
                sources = []
            if state is not None and (not isinstance(sources, list) or not sources or
                    any(not isinstance(fid, str) or fid not in facts or facts[fid]["trialId"] != tid for fid in sources)):
                issues.append({"code": "invalid_membership", "groupId": group, "trialId": tid})
                assignment[tid] = None
    # Check containment after validating the first group, regardless of JSON key order.
    for group, assignment in members.items():
        for tid, state in assignment.items():
            if group != "group_1" and state is True and members.get("group_1", {}).get(tid) is not True:
                ...
                assignment[tid] = None
    metrics = {}
    metric_entries = work.get("metrics", [])
    if not isinstance(metric_entries, list):
        metric_entries = []
        issues.append({"code": "unsupported_metric", "id": ""})
    ...
    assessments = work.get("assessments", [])
    covered = {(a.get("analysisId"), a.get("groupId")) for a in (assessments if isinstance(assessments, list) else [])
               if isinstance(a, dict) and isinstance(a.get("analysisId"), str) and isinstance(a.get("groupId"), str)
               and a.get("disposition") in {"contributes", "not_applicable", "insufficient_evidence"} and a.get("reason")}
...
def chart(metric):
    ...
    for row in rows:
        ...
        parts.append('<div class="chart-row"><span class="chart-label">' + html.escape(row["label"]) +
                     ...
                     html.escape(value) + (f'<small> (n={row["n"]})</small>' if metric["method"] not in {"count", "percentage"} else "") + "</span></div>")
    if any(row["n"] < 5 for row in rows):
        parts.append('<figcaption>Results with fewer than five contributing trials are descriptive precedents.</figcaption>')
