...
class ManagedAgent:
    ...
    async def create_idle(self, run_id: str, intent: str, instructions: str, files: dict[str, bytes]):
        self.config.validate(self.settings)
        # Larger snapshots must use Files API references, never silently truncate.
        if len(files) > 50 or sum(map(len, files.values())) > 10 * 1024 * 1024:
            raise ManagedAgentError("AGENT_INPUT_REQUIRES_FILE_UPLOAD")
        inputs = []
        for name, data in files.items():
            if not re.fullmatch(r"[a-zA-Z0-9_.-]+", name) or len(data) > 5 * 1024 * 1024:
                raise ManagedAgentError("AGENT_INPUT_REQUIRES_FILE_UPLOAD")
