...
class MaxAgentWorker:
    ...
    async def session(self, job):
        ...
        if not found:
            ...
            found = await self.agent.create_idle(run_id, intent, INSTRUCTIONS, files)
    ...
    async def analyze(self, job):
        ...
        current = next((t for t in turns if t["id"] != cp["previousTurnId"]), None)
        # Only turns newer than the checkpoint can be associated with this request.
        if turns and turns[0]["id"] == cp["previousTurnId"]:
            current = None
        if not current:
