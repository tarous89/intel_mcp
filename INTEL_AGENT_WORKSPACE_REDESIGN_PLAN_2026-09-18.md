...
| [S19](#s19--deploy-the-integrated-candidate-for-the-user-led-pilot) | Deploy the integrated candidate for the user-led pilot | IN_PROGRESS | Restricted owner pilot is live. First discovery POST exposed a Render custom-domain origin mismatch; App hotfix `ad64f12` / deploy `dep-dan65ofavr4c73a4uq9g` is live and all hotfix CI is green. No job/model turn was created by the failed attempt. Awaiting retry and first complete real report. |
...
**Completion record:** App `4e97aaecf47f6ad7a3b372bfc8893cb657216376` passes all five CI suites. 24 PGlite/PostgreSQL test entries verify owned/versioned exports, service/session/origin gates, replay, revoked Max access, verified downloads and unchanged usage/HTML. Nine worker export/PDF tests verify slot cleanup, exact order and lossless deterministic XLSX. Desktop/mobile flows cover lost replies, derivative retries, selected order, Light/Max controls and exhausted allowance; screenshots inspected. Engine artifact interface reused without change. Default-off and undeployed.
...
**Completion record:** IN_PROGRESS. Engine merge `e606612c057019927fa181d032977bec3db264f4` is live with migration 045 and HTTP 200 health. App runtime merge `035c2422df3909f95ad3d910e3769a373cc343f3` is live; migrations 0027–0031 applied. Restricted pilot deploy `dep-dan5ud3m8hqs73a5u0u0` is live on App main `22b5f71` (documentation-only commits after the verified runtime merge). Startup passed Max/Workspace export readiness, Engine catalogue/health, App health and Workspace queue probes without starting a model turn. `INTEL_WORKSPACE_ENABLED=true`; the owner's exact authenticated App UUID is the sole pilot allowlist entry and new work is enabled for that account only. Pre-run whole-service Render memory is ~171 MiB of 512 MiB. S19 cannot be DONE until the user completes the first real report so clinical usefulness, HTML/PDF quality, latency, actual cost and whole-service production peak memory are recorded. First user discovery was blocked pre-queue by direct browser-Origin vs internal Render-origin comparison. PR #155 / App `ad64f1208438ef605a175eac7da39f1a6cf56960` adds a proxy-aware same-origin guard across all Workspace mutations, passed Workspace/Managed-Max/Public/Report-dataset CI, and is live as `dep-dan65ofavr4c73a4uq9g`. The failed attempt created no discovery job/model turn; retry is pending.
